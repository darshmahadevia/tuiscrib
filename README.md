...
The terminal client opens the keyboard-only shell. Press `b` for Boards, `s` to sign in, `r` to register, `x` to sign out, `?` for help, or `q` to quit. In the Board list, `c` creates a Board, `j` redeems a Join Code, `f` filters Memberships by Board name, and `[/]` selects the Board for actions. A Member can use `a`, then `l`, to leave after confirmation; the Owner cannot leave. A created Board receives a grouped initial Join Code displayed only in that creation result; the Tuiscrib Service stores only its verification hash. A valid Terminal Session is restored from the protected platform config location on launch and expires after 30 days without authenticated activity. The shell requires an 80 by 24 terminal and supports Unicode plus a 256-color baseline, with truecolor used when detected.
